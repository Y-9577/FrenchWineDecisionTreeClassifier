...
df.columns = ['species','alcohol','malic_acid','alcalinity ash','ash','magnesium']
# 查看前5条数据
df.head()
print(df.head()) 
# 查看数据描述性统计信息
...
plt.figure(figsize=(20,5))
plt.subplot(131)
scatter_plot_by_category('species', 'magnesium', 'malic_acid')
...
# 使用训练的模型进行预测，为了方便，
# 案例直接把测试集里面的数据拿出来三条
print(X_test[0:4])#利用3个数据进行测试，即取3个数据作为模型的输入端
model.predict(X_test[0:4])
print(model.predict(X_test[0:4]))#输出测试的结果，即输出模型预测的结果
